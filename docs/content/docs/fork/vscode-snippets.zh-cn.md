...
本 fork 提供 `.vscode/hextra.code-snippets` —— 49 个手写的 VS Code 代码片段，覆盖每一个 Hextra shortcode、布局所读取的页面 front matter 键，以及渲染钩子能识别的代码围栏属性。在 VS Code 中打开仓库即可立即使用；不需要扩展，也不需要配置。
