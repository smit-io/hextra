...
このフォークには `.vscode/hextra.code-snippets` が同梱されています。Hextra のすべてのショートコード、レイアウトが読み取るページのフロントマターキー、レンダーフックが解釈するコードフェンス属性をカバーする、手書きの 49 個の VS Code スニペットです。リポジトリを VS Code で開けばすぐに使えます。拡張機能も設定も不要です。
