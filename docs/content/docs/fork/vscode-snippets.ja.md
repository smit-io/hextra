...
このフォークには `.vscode/hextra.code-snippets` が同梱されています。Hextra のすべてのショートコード、レイアウトが読み取るページのフロントマターキー、レンダーフックが解釈するコードフェンス属性をカバーする、手書きの 89 個の VS Code スニペットです。リポジトリを VS Code で開けばすぐに使えます。拡張機能も設定も不要です。

<!--more-->

## なぜ必要か

Hextra の機能は、ドキュメントを読まないと見つけられません。ページを書くということは、カードのタグ色が（`tagType` ではなく）`tagColor` であること、モザイクギャラリーの項目が `span="wide"` を取ること、行ハイライトが `hl_lines=[2,4]` であること、そして 5 つのショートコードには通常の山括弧ではなく[パーセント記法](#notation-is-not-uniform)が必要なことを覚えておくということです。スニペットはそのすべてを、プレフィックスと `Tab` キーの向こう側に隠してくれます。
...
5 つのショートコードは、内側のコンテンツを Markdown としてレンダリングするためにパーセントデリミタを使います。残りは山括弧を使います。これを間違えると、コンテンツがリテラルテキストとしてレンダリングされてしまいます。スニペットにはそれぞれ正しい記法が組み込まれています：
...
これらのスニペットは [`skills/hextra/`](https://github.com/smit-io/hextra/tree/main/skills/hextra) にも供給されます。これはコーディングエージェントに Hextra サイトの執筆方法を教えるスキルパッケージです。そこではショートコードのテンプレートが信頼できる情報源であり、`npm run build:skill` はパラメータ名・デフォルト値・必須かどうかを `layouts/_shortcodes/` から直接読み取ります。スニペットは、テンプレートでは表現しにくい列挙値とコピー&ペースト可能な例に使われます。ジェネレーターは両者を突き合わせ、どのテンプレートも読まないパラメータを含むスニペットを報告するため、手書きのスニペットが実装から静かに乖離することはありません。
