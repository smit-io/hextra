...
| `make skill`     | `skills/hextra/` のスキルリファレンスを再生成し、`VERSION` から `.claude-plugin/*.json` にバージョンを刻む |
...
| ターゲット                        | 内容                                                                                                                                      |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `make verify`                     | すべてを順に実行 — 整形と再生成のあと、再チェック・ビルド・スイート実行。コミット前に使うコマンド                                         |
| `make test`                       | 先に `fmt-check` と `skill-check`、そのあとドラフトを含まない新規本番ビルドに対する Playwright スイート一式                               |
...
| `make fmt-check`                  | 何も書き換えずに整形を検証 — CI が実行するもの                                                                                            |
| `make skill-check`                | 生成されたスキルリファレンスとプラグインマニフェストが最新かを検証                                                                        |
| `make report`                     | 直近の Playwright HTML レポートを [localhost:9323](http://localhost:9323) で配信                                                          |

### リリース

| ターゲット                 | 内容                                                             |
| -------------------------- | ---------------------------------------------------------------- |
| `make bump VERSION=0.21.2` | `VERSION` を設定し、`.claude-plugin/*.json` に同じ番号を刻み直す |

リポジトリ直下の `VERSION` はツリー内で唯一のバージョン番号であり、公開の引き金を引く唯一のファイルでもあります。これを変更するコミットが `main` に入ると `v<VERSION>` のタグが付き、GitHub リリースが作られます。`bump` はファイルの書き換えとマニフェストの再生成を 1 ステップで行い、書式の誤ったバージョン、すでに設定済みのバージョン、タグが存在するバージョンを拒否します。編集するのはファイルだけで、コミットがマージされるまで何も公開されません。リリースノートは公開前に `npm run changelog` で確認しておいてください。
