...
| 項目                   | アップストリーム                        | このフォーク                                                                |
| ---------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| ボーダー               | なし                                    | 1px ボーダー：`neutral-400`（ライト）、`neutral-700`（ダーク）              |
| 角丸                   | `rounded-xl`                            | `rounded-sm`                                                                |
| 背景                   | 半透明のプライマリ色（`primary-700/5`） | ソリッドなパレット色：`neutral-50` / `neutral-950`                          |
| ファイル名ヘッダー     | プライマリ色ベース、`rounded-t-xl`      | ボーダー付きのパレット背景（`neutral-200` / `neutral-800`）、`rounded-t-sm` |
...
ファイル名バーは、独自のボーダーとパレット背景（ライトは `neutral-200`、ダークは `neutral-800`）を持ち、ブロックの上にぴったり重なって、視覚的にはエディタのタブのように見えます。
