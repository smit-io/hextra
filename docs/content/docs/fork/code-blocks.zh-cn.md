...
| 方面         | 上游                              | 本 fork                                                             |
| ------------ | --------------------------------- | ------------------------------------------------------------------- |
| 边框         | 无                                | 1px 边框：`neutral-400`（浅色）、`neutral-700`（深色）              |
| 圆角         | `rounded-xl`                      | `rounded-sm`                                                        |
| 背景         | 半透明主色染色（`primary-700/5`） | 实色调色板：`neutral-50` / `neutral-950`                            |
| 文件名标题栏 | 主色染色、`rounded-t-xl`          | 调色板背景（`neutral-200` / `neutral-800`）并带边框、`rounded-t-sm` |
...
文件名栏紧贴在代码块顶部，拥有自己的边框和调色板背景（浅色为 `neutral-200`，深色为 `neutral-800`），视觉上就像一个编辑器标签页。
