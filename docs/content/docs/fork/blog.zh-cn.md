...
分页导航自身的开关优先级更高，因为它们是显式声明的，而是否属于某个系列则不是：`displayPagination: false` 会同时隐藏两者；在 front matter 中设置了 `prev`/`next`（包括 `prev: false`）的文章，仍保留它自己配置的分页导航。不过系列顺序会覆盖 `reversePagination`——后者只是把同一批文章按日期排列。
