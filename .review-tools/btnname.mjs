...
const s=createServer(async(q,r)=>{let f=join(root,decodeURIComponent(q.url.split("?")[0]));try{if((await stat(f)).isDirectory())f=join(f,"index.html")}catch{r.writeHead(404).end();return}try{r.writeHead(200,{"content-type":T[extname(f)]||"application/octet-stream"}).end(await readFile(f))}catch{r.writeHead(404).end()}});
...
const nodeFor=await btn.evaluate(el=>{el.setAttribute("data-axprobe","1");return 1;});
...
for(const n of btnNodes.filter(n=>/^(Copy Page|Copy as Markdown|Toggle page context menu)$/.test(n.name?.value||""))) console.log("  button name:",JSON.stringify(n.name?.value),"description:",JSON.stringify(n.description?.value));
